...
function buildSimple(raw) {
    ...
    const libreRamPct = memTotal > 0 ? round1((memFree / memTotal) * 100) : null;
    ...
    return {
        ...
        carga1m: Array.isArray(host.loadavg_1_5_15) ? host.loadavg_1_5_15[0] : null,
...
async function collectSatMetrics(options) {
    ...
    const includeProcessCpu = options.includeProcessCpu !== false; // default true
    const processCpuSampleMs = options.processCpuSampleMs ?? 150;
    const returnRaw = options.returnRaw !== false; // default true
